...
class TestimonialForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # RichTextField подставляет виджет CKEditor и игнорирует Meta.widgets,
        # поэтому заменяем его здесь: на публичной форме нужен обычный textarea
        self.fields['text'].widget = forms.Textarea(attrs={
            'class': 'form-input',
            'rows': 6,
            'placeholder': 'Расскажите, с чем обращались и как прошло лечение',
        })
        self.fields['age'].widget.attrs.update({'min': 18, 'max': 110})
